...
router.post(
  ...
  async (req, res, next) => {
    if (
      !_.some(req.user.permissions, (pm) =>
        _.includes(['write:assets', 'manage:system'], pm)
      )
    ) {
    ...
    // Sanitize filename and add a unique string
    const uniqueSuffix = `_${uuidv4()}`
    const extension = path.extname(fileMeta.originalname)
    const baseName = path.basename(fileMeta.originalname, extension)
    fileMeta.originalname = sanitize(
      `${baseName.toLowerCase().replace(/[\s,;#]+/g, '_')}${uniqueSuffix}${extension}`
    )
    ...
    if (
      !WIKI.auth.checkAccess(req.user, ['write:assets'], { path: assetPath })
    ) {
      return res.status(403).json({
        succeeded: false,
